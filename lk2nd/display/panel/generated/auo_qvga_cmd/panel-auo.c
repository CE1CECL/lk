...
static inline struct auo *to_auo(struct drm_panel *panel)
{
	return container_of_const(panel, struct auo, panel);
}

static void auo_reset(struct auo *ctx)
{
...
static int auo_on(struct auo *ctx)
{
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0x00, 0x04);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xf0,
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xb6, 0x53, 0x53, 0x53);
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xb7, 0x33, 0x33, 0x33);
	...
	mipi_dsi_dcs_write_seq_multi(&dsi_ctx, 0xcf,
				     0xff, 0xd4, 0x95, 0xef, 0x4f, 0x00, 0x04);
...
static int auo_off(struct auo *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static const struct drm_display_mode auo_mode = {
...
{
	return drm_connector_helper_get_modes_fixed(connector, &auo_mode);
}
...
static int auo_bl_update_status(struct backlight_device *bl)
{
...
static int auo_bl_get_brightness(struct backlight_device *bl)
{
	struct mipi_dsi_device *dsi = bl_get_data(bl);
	u16 brightness;
	int ret;
...
static const struct backlight_ops auo_bl_ops = {
	.update_status = auo_bl_update_status,
	.get_brightness = auo_bl_get_brightness,
};

static struct backlight_device *
auo_create_backlight(struct mipi_dsi_device *dsi)
{
...
static int auo_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct auo, panel,
				   &auo_panel_funcs,
	...
	if (!ctx->panel.backlight) {
		ctx->panel.backlight = auo_create_backlight(dsi);
...
static void auo_remove(struct mipi_dsi_device *dsi)
{
	struct auo *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
MODULE_DESCRIPTION("DRM driver for AUO qvga command mode dsi panel");
MODULE_LICENSE("GPL");
