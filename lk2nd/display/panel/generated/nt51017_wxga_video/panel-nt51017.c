...
{
	return container_of_const(panel, struct nt51017, panel);
}
...
static int nt51017_bl_update_status(struct backlight_device *bl)
{
...
static int nt51017_bl_get_brightness(struct backlight_device *bl)
{
	struct mipi_dsi_device *dsi = bl_get_data(bl);
	u16 brightness;
	int ret;
...
static const struct backlight_ops nt51017_bl_ops = {
	.update_status = nt51017_bl_update_status,
	.get_brightness = nt51017_bl_get_brightness,
};

static struct backlight_device *
nt51017_create_backlight(struct mipi_dsi_device *dsi)
{
...
static int nt51017_probe(struct mipi_dsi_device *dsi)
{
	...
	if (!ctx->panel.backlight) {
		ctx->panel.backlight = nt51017_create_backlight(dsi);
